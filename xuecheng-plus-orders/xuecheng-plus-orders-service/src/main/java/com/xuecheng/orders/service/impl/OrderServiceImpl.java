...
public class OrderServiceImpl implements OrderService {
    ...
    public PayStatusDto queryPayResultFromAlipay(String payNo) {
        ...
        try {
            ...
            throw new XueChengPlusException("请求支付宝查询支付结果异常");
        }
        // 3. 查询失败
...
// 1. 获取支付流水号
        ...
        if (payRecord == null) {
           throw new  XueChengPlusException("未找到支付记录");
        }
        XcOrders order = xcOrdersMapper.selectById(payRecord.getOrderId());
        if (order == null) {
           throw new  XueChengPlusException("找不到相关联的订单");
        }
        ...
        if ("TRADE_SUCCESS".equals(tradeStatus)) {
            ...
            if (updateRecord <= 0) {
                throw new XueChengPlusException("更新支付交易表失败");
            }
            // 更新订单表
            ...
            if (updateOrder <= 0) {
                log.debug("更新订单表失败");
                throw new XueChengPlusException("更新订单表失败");
            }
        }
    ...
    public XcOrders saveOrders(String userId, AddOrderDto addOrderDto) {
        ...
        if (insert <= 0) {
            throw new XueChengPlusException("插入订单记录失败");
        }
        // 3. 插入订单明细表
        ...
        xcOrdersGoodsList.forEach(goods -> {
            ...
            if (insert1 <= 0) {
                throw new XueChengPlusException("插入订单明细失败");
            }
        });
        return order;
    }
    ...
    public XcPayRecord createPayRecord(XcOrders orders) {
        ...
        if (insert <= 0) {
           throw new XueChengPlusException("插入支付交易记录失败");
        }
        return payRecord;
    }
    @Override

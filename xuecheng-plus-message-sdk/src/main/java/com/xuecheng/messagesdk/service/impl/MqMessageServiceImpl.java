...
public class MqMessageServiceImpl extends ServiceImpl<MqMessageMapper, MqMessage> implements MqMessageService {
    ...
    public MqMessage addMessage(String messageType, String businessKey1, String businessKey2, String businessKey3) {
        ...
        queryWrapper.eq(!StringUtils.isBlank(businessKey1),MqMessage::getBusinessKey1,businessKey1)
                .eq(!StringUtils.isBlank(businessKey2),MqMessage::getBusinessKey2,businessKey2)
                .eq(!StringUtils.isBlank(businessKey3),MqMessage::getBusinessKey3,businessKey3)
                .eq(MqMessage::getMessageType,messageType);
        MqMessage message = mqMessageMapper.selectOne(queryWrapper);//查看数据库中数据
        int flag;
        //若数据库已经有数据则进行更新否则插入
        if(message==null){
            flag = mqMessageMapper.insert(mqMessage);
        }else{
            flag = mqMessageMapper.update(mqMessage, queryWrapper);
        }
        if(flag>0){
            return mqMessage;
        }else{
